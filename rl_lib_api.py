...
# Get Alert List
def daily_api_alert_list_get(rl_settings,requestseverity):
    action = "GET"
    url = "https://" + rl_settings['apiBase'] + '/alert?alert.status=open%s&policy.id=c68d98d2-5c4f-48fe-814a-28d7e560ebc5&timeType=relative&timeAmount=24&timeType=relative&timeUnit=hour&detailed=true' % requestseverity
    return rl_call_api(action, url, rl_settings)

def weekly_api_alert_list_get(rl_settings,requestseverity):
    action = "GET"
    url = "https://" + rl_settings['apiBase'] + "/alert?alert.status=open%s&timeType=relative&timeAmount=1&timeType=relative&timeUnit=week&detailed=true" % requestseverity
    return rl_call_api(action, url, rl_settings)

def monthly_api_alert_list_get(rl_settings,requestseverity):
    action = "GET"
    url = "https://" + rl_settings['apiBase'] + "/alert?alert.status=open%s&timeAmount=1&timeType=relative&timeUnit=month&detailed=true" % requestseverity
    return rl_call_api(action, url, rl_settings)

def alltime_alert_list_get(rl_settings,group):
    action = "GET"
    url = "https://" + rl_settings['apiBase'] + "/alert?account.group=%s&alert.status=open&timeType=to_now&timeUnit=epoch&detailed=true" % group
    return rl_call_api(action, url, rl_settings)
